...
def install_root_from_ubuntu_tarball(localSetupFile):
	...
	commands += f'    && curl -S --location-trusted --progress-bar --retry 4 https://root.cern/download/{root_file} -O  --cacert {jcertificate}\\\n'
...
def install_meson():
	...
	commands += f'    && curl -S --location-trusted --progress-bar --retry 4 {meson_location}/{meson_file} -O --cacert {jcertificate}  \\\n'
